...
# entered by the user.
        ...
        numericInput("tmb_in",  label = h3("TMB", style ="font-size:100%; font-weight: bold"), value = " ", min = 0),    
...
#### SPG addition to example APP
# Class "error" references above CSS
        tags$div(class='error',textOutput('badInput1a')),
        tags$div(class='error',textOutput('badInput1')),
        tags$div(class='error',textOutput('badInput1b')),
        tags$div(class='error',textOutput('badInput2')),
        tags$div(class='error',textOutput('badInput3')),
        tags$div(class='error',textOutput('badInput4')),
        tags$div(class='error',textOutput('badInput5')),      
        textOutput("hrtext"),
...
# badInput is an error message that is displayed when the inputs are invalid
    ...
    output$badInput1a <- renderText({
        if (isTruthy(input$pmc_in) & isTruthy(input$tmb_in)) {
            badInput1a <- "You must not give a value for both SM Count and TMB"
        }
    })
    output$badInput1 <- renderText({
        if ((!(isTruthy(input$pmc_in)) & !(isTruthy(input$tmb_in))) | !(isTruthy(input$cnaf_in))) {
            badInput1 <- "CNAF must be > 0 & <= 1 and either SM count must be >= 1 or TMB must be > 0.034"
        }    
        else {    
#                  (!(isTruthy(input$pmc_in) & isTruthy(input$tmb_in))) &
            if (!(
                   (input$pmc_in >= 1      | !(isTruthy(input$pmc_in))) & 
                   (input$tmb_in >= 0.0336 | !(isTruthy(input$tmb_in))) & 
                  input$cnaf_in >= 0 & input$cnaf_in <= 1)) {
                badInput1 <- "CNAF must be > 0 & <= 1 and either SM count must be >= 1 or TMB must be >0.034"
            }
        }
    })
    ...
    output$hrtext <- renderText({
        if (!(isTruthy(input$pmc_in)) | !(isTruthy(input$tmb_in))) {
            if ((isTruthy(input$pmc_in)) & (isTruthy(input$cnaf_in)) |
                (isTruthy(input$tmb_in)) & (isTruthy(input$cnaf_in))) { 
                if ( (isTruthy(input$pmc_in)) & (isTruthy(input$cnaf_in)) & 
                    ...
                    if ( (isTruthy(input$tmb_in)) & (isTruthy(input$cnaf_in)) &
                        (input$tmb_in >= 0.03356 & input$cnaf_in >= 0 & input$cnaf_in <= 1)
                        & 
                        input$cancer_in == "Not chosen" & input$stage_in == "Not chosen"  
                        & input$grade_in == "Not chosen" & !isTruthy(input$age_in) ) {
                        hrtext <- "Mortal hazard relative to patients with TMB = 0.034 and CNA fraction = 0 "
                    }
                }     
            }
        }    
        ...
        if (!(isTruthy(input$pmc_in)) | !(isTruthy(input$tmb_in))) {
            if ((isTruthy(input$pmc_in)) & (isTruthy(input$cnaf_in)) |
                ...
                if (((isTruthy(input$pmc_in)) & (isTruthy(input$cnaf_in))
                     ...
                     |
                     ((isTruthy(input$tmb_in)) & (isTruthy(input$cnaf_in))
                     & (input$tmb_in < 0.0336 | input$cnaf_in < 0 | input$cnaf_in >= 1))  )
                    { 
                    hr <- " "
                }   
                else {
...
# lower and upper bound of the 95% CI are given in positions 3 and 4, respectively.
    ...
    # Check that TMB covariates are given and are valid
        if (!(isTruthy(input$pmc_in)) | !(isTruthy(input$tmb_in))) {
          if (  input$cnaf_in >= 0 &  input$cnaf_in <= 1 &((isTruthy(input$pmc_in)) & input$pmc_in >= 1 )
                |((isTruthy(input$tmb_in)) & input$tmb_in >= 0.0336)) {    
   ...
   #@@@@@@@@@@ Plot survival curve when TMB covariates are valid, age is missing and cancer, stage and grade are not chosen @@@@        
                if (input$cancer_in == "Not chosen" & !isTruthy(input$age_in)) { 
                    ...
                    if (input$stage_in == "Not chosen"  & input$grade_in == "Not chosen") {
                        ...
                        p <- survplot(model, logPM = log(pmc_in), cnaf = input$cnaf_in, conf.int=.95,col='red', col.fill='yellow', 
...
# Regress fate against logPM cancer.f stage.f and cnaf
